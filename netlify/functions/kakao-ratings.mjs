...
async function fetchKakaoSearch(place) {
  ...
  for (const q of queries) {
    ...
    const match = sameId || (sameName && distanceMeters(place, {
      x: numberOrNull(sameName?.lon),
      y: numberOrNull(sameName?.lat)
    }) < 1000);

    if (match) {
      const rating = numberOrNull(match.rating_average ?? match.ratingAverage);
      const reviewCount = numberOrNull(match.reviewCount ?? match.review_count ?? match.ratingCount);
      return {
        id: place.id,
        rating,
        reviewCount,
...
export default async function handler(request) {
  ...
  try {
    ...
    if (!places.length) {
      return json({ places: [], filtered: 0, source: "kakaomap" }, 200, origin);
    }
